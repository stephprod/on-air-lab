...
$(document).on("click", "a.booking-for-payment-chat", on_reservation_for_payment_link_click);
$(document).on("click", "a.meet-up-for-payment-chat", on_reservation_for_payment_link_click);
...
actions.paiement.push(function (data){
  ...
  //console.log(data);
  // var accept_target = data.pro_type == 3 ? "#rdv-for-payment-modal" : "#booking-for-payment-modal";
  ...
  // }else{
    ...
    ret += '</p>';
    ret += '<div class="div-submi" style="color: black;"><a href="#" class="btn-refus">refuser</a><a href="#" data-toggle="modal" class="btn-accept">accepter</a></div><div class="corner"></div><span>'+data.created+'</span>';
  //}
  ret += '</div></div></div>';
  //ret += 'statut de la demande ('+data.request_state+')</div>';
  //console.log(data.request_state);
...
$(document).on("click", ".friend[data-action='room']", function(){
  ...
  var video_modal = '<a class="module" href="#video-modal" data-toggle="modal">video</a>';
  $("#profile p").html(name);
  ...
  if ($(this).data('coresp-type') == "4"){
    ...
    $(".box-module").append(rdv_modal);
    $(".box-module").append(payment_modal);
    $(".box-module").append(song_modal);
    $(".box-module").append(video_modal);
  ...
  // }
    ...
    $(".box-module").append(booking_modal);
    $(".box-module").append(song_modal);
  ...
  }else if($(this).data('coresp-type') == "3"){
    $(".box-module").append(rdv_modal);
    $(".box-module").append(song_modal);
    $(".box-module").append(video_modal);
    // $($(".box-module .module")[0]).hide();
...
function on_refund_link_click(){
  ...
  date_presta =  new Date(parent.data("date-presta"));
  date_payment = new Date(parent.data("date-payment"));
  fees = (parseFloat(parent.data("price-payment")) * 0.03) + 0.3;
  var confirm_text = "Êtes-vous sûr de vouloir annuler cette réservation ?";
  // console.log(now);
  if (parent.data("type-transac") == "MOD"){
    if (Date.getDaysBetween(date_presta, date_payment) <= 1){
      // console.log(Date.getMinutesBetween(date_presta, now));
      if (Date.getMinutesBetween(date_presta, now) <= 30){
        amount_refund = (parseFloat(parent.data("price-payment")) - fees) / 2;
        mode = 1;
      }else{
        amount_refund = (parseFloat(parent.data("price-payment")) - fees);
        mode = 0;
      }
    }else{
      if (Date.getDaysBetween(date_presta, now) <= 1){
        amount_refund = (parseFloat(parent.data("price-payment")) - fees) / 2;
        mode = 1;
      }else{
        amount_refund = (parseFloat(parent.data("price-payment")) - fees);
  ...
  if (r){
    $.ajax({
        ...
        url: "/annule_resa",
        data: {
          action: "art_annulation",
          mode: mode,
          id_payment: parent.data("id-payment"),
          id_type_message: parent.data("id-type-message"),
          type_transac: parent.data("type-transac"),
          date_presta: parent.data("date-presta"),
          date_payment: parent.data("date-payment"),
          price_refund: amount_refund,
          id_pro: parent.data("id-pro"),
          id_art: parent.data("id-art")
        },
...
function chat_headlines_span_click(e){
  ...
  if (target.hasClass("history")){  
    ...
    payments_part.css("background", "");
    window.parent.$("#chat_title").html("Mon Chat");
    $("div#friends").show();
  }else{
    ...
    notifs_part.css("background", "");
    window.parent.$("#chat_title").html("Mes Réservations");
    $("div#chats").show();
  }
}
function on_socket_refresh_art_payments(payments){
  ...
  if (payments instanceof Array){
    for (var i = 0; i < payments.length; i++){
      ...
      var status = "inactive", actions, start, date_p;
      ...
      if (payments[i]["state_payment"] == "valide")
        ...
        status = "away";
      if (new Date(payments[i]["start"]) >= new Date()){
        actions = '<p><a href="#" id="refund" class="btn-refus">Annuler la réservation</a></p>';
      }else{
      ...
      $("div#chats").append('<div class="friend" style="height:auto;" '+
        ...
        'data-id-art="'+payments[i]["id_artist"]+'" '+
        'data-id-pro="'+payments[i]["id_pro"]+'" '+
        'data-price-payment="'+payments[i]["price_payment"]+'" '+
        'data-date-presta="'+payments[i]["start"]+'" >'+
        '<p>'+
          '<strong>'+payments[i]["desc_payment"]+' ('+payments[i]["type_payment"]+')</strong> <span>'+payments[i]["price_payment"]+' €</span><p class="preview">Date du paiement : '+date_p_for_display+'<br>Date de la prestation : '+start_for_display+'<br>Type de la transaction : '+type_transac+'</p>'+
          actions+
        '</p>'+
...
function check_abo(usr){
  if (usr != "null" && (usr.type == 2 || usr.type == 3)){
    if ((usr.abonnement !== undefined && usr.abonnement) || roomDisplay == 1)
      return true;
    else
      return false;
  }
...
function on_msg_send_click(){
  ...
  if(check_abo(user)){
    ...
    var ret = {success: [false], errors: {message: ["identification ou abonnement requis !"]}, global_msg: ['Vous devez être connecté et avoir un abonnement en cours de validité pour utiliser le chat !']};
    update_front_with_errors(ret.errors);
    update_front_with_msg(ret, "msg-inf-chat");
  }
}
...
function on_reservation_for_payment_link_click(e){
  e.preventDefault();
  ...
  global_datas.user_receiv = user_receiv.id_coresp;
  global_datas.user_sender = user.id;
  if ($(this).hasClass("meet-up-for-payment-chat")){
    modal = $("#rdv-for-payment-modal");
    resp_target = "msg-rdv-payment";
...
function on_form_song_submit(e){
  e.preventDefault();
    ...
    if (!check_abo(user)){
      var ret = {success: [false], global_msg: ['Vous devez être connecté et avoir un abonnement en cours de validité pour utiliser les modules du chat !']};
      update_front_with_msg(ret, "msg-song");
      return false;
    }
    $.ajax({
...
function on_video_module_send_link_click(e){
  ...
  if (!check_abo(user)){
    var ret = {success: [false], global_msg: ['Vous devez être connecté et avoir un abonnement en cours de validité pour utiliser les modules du chat !']};
    update_front_with_msg(ret, "msg-video");
    return false;
  }
  $.ajax({
    type: "POST",
...
function on_devis_module_send_link_click(e){
  ...
  if (user.type == 4){
    ...
    if (!check_abo(user)){
      var ret = {success: [false], global_msg: ['Vous devez être connecté et avoir un abonnement en cours de validité pour utiliser les modules du chat !']};
      update_front_with_msg(ret, "msg-devis");
      return false;
    }
    $.ajax({
      type: "POST",
...
function on_socket_update_paymentstypemessage(data){
  ...
  div.attr("request-id", data.payment.id)
...
function on_module_payment_bank_link_click(e){
  ...
  var state = false, buttonText = "", button,
   buttonClass, new_state, newButtonClass, msg = "En activant le module bancaire vous acceptez que Label-Onair vous prélève directement 3% du montant de votre demande et le montant de votre demande doît être supérieur ou égale à 20€ !";
  // console.log(e.target);
